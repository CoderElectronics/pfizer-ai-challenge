...
while cap.isOpened():
    ...
    try:
        rv_diff = cv2.absdiff(frame1, frame2) # To find out absolute difference of first frame and second frame
...
for item in enumerate(tr_time_pts):
    print(item)

def preview_3d(xypts):
    coordinates = [(i["X"], i["Y"]) for i in xypts]
    timestamp = [z["Ts"] for z in xypts]
